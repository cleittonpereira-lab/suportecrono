 ...
 * de Módulo de Resiliência (MR). Guarda o estado editável (sample) por escopo
 * (`ensaio.id` ou "local") sincronizando em tempo real com controle de concorrência.
 */
import type { ModuloResilienciaSample } from "./types";
...
const KEY = (scopeId: string) => `modulo-resiliencia:draft:${scopeId}`;

export type MRDraft = {
  sample: ModuloResilienciaSample;
  photos?: any[];
  savedAt: string;
...
export function loadDraft(scopeId: string): Partial<MRDraft> | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(KEY(scopeId));
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<MRDraft>;
...
export function saveDraft(
  ...
  const timer = setTimeout(() => {
    ...
    trackSave(() =>
      ...
      .catch((err) => {
        console.warn("[MR saveSharedDraft] Falha na sincronização:", err);
      });
  }, 400);
  ...
  try {
    ...
    const res = await loadSharedDraft({
      data: {
        ...
        osNum: opts?.osNum,
        amCode: opts?.amCode,
        ensaioTipo: opts?.ensaioTipo || "modulo-resiliencia",
      },
    });
    if (res?.success && res.payload) {
      ...
      const payloadWithRev = { ...(res.payload as object), rev: res.rev } as Partial<MRDraft>;
      try {
    ...
    console.warn("[MR fetchRemoteDraft] Falha ao carregar rascunho remoto:", err);
  }
  return null;
}
