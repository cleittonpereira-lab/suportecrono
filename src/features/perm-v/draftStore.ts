 ...
 * trabalho do ensaio de Permeabilidade a Carga Variável (PERM.V). Guarda o
 * estado editável (sample) por escopo (`ensaio.id` ou "local") sincronizando
 * em tempo real com controle de concorrência.
 */
...
const KEY = (scopeId: string) => `perm-v:draft:${scopeId}`;

export type PermVDraft = {
  sample: PermVSample;
  photos?: any[];
  savedAt: string;
...
export function loadDraft(scopeId: string): Partial<PermVDraft> | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(KEY(scopeId));
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<PermVDraft>;
...
export function saveDraft(
  ...
  const timer = setTimeout(() => {
    ...
    trackSave(() =>
      ...
      .catch((err) => {
        console.warn("[PERM.V saveSharedDraft] Falha na sincronização:", err);
      });
  }, 400);
...
): Promise<Partial<PermVDraft> | null> {
  try {
    ...
    const res = await loadSharedDraft({
      data: {
        ...
        osNum: opts?.osNum,
        amCode: opts?.amCode,
        ensaioTipo: opts?.ensaioTipo || "perm-v",
      },
    });
    if (res?.success && res.payload) {
      ...
      const payloadWithRev = { ...(res.payload as object), rev: res.rev } as Partial<PermVDraft>;
      try {
    ...
    console.warn("[PERM.V fetchRemoteDraft] Falha ao carregar rascunho remoto:", err);
  }
  return null;
}
