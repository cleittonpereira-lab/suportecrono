...
type CatalogKind = "equipments" | "operators" | "typists" | "rings";

const KEY = "suporte-infra:cd-catalogs";
const DEFAULTS: Record<CatalogKind, string[]> = {
  equipments: ["Cisalhamento Direto"],
  operators: ["Técnico Exemplo"],
  typists: ["Técnico Exemplo"],
  rings: ["ANEL-01", "ANEL-02", "ANEL-03"],
};
...
function read(): State {
  ...
  try {
    ...
    return {
      ...
      typists: unique([...(parsed.typists ?? []), ...DEFAULTS.typists]),
      rings: unique([...(parsed.rings ?? []), ...DEFAULTS.rings]),
    };
  } catch {
    return { ...DEFAULTS };
  }
