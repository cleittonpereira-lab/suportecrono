 ...
 * trabalho do Point Load Test. Mesmo padrão de compressao-simples/draftStore.ts.
 */
import type { PLTSample } from "./types";
...
const KEY = (scopeId: string) => `load-test:draft:${scopeId}`;

export type PLTDraft = {
  sample: PLTSample;
  photos?: any[];
  savedAt: string;
...
export function loadDraft(scopeId: string): Partial<PLTDraft> | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(KEY(scopeId));
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<PLTDraft>;
...
export function saveDraft(
  ...
  const timer = setTimeout(() => {
    ...
    trackSave(() =>
      ...
      .catch((err) => {
        console.warn("[Point Load Test saveSharedDraft] Falha na sincronização:", err);
      });
  }, 400);
  ...
  try {
    ...
    const res = await loadSharedDraft({
      data: {
        ...
        osNum: opts?.osNum,
        amCode: opts?.amCode,
        ensaioTipo: opts?.ensaioTipo || "load-test",
      },
    });
    if (res?.success && res.payload) {
      ...
      const payloadWithRev = { ...(res.payload as object), rev: res.rev } as Partial<PLTDraft>;
      try {
    ...
    console.warn("[Point Load Test fetchRemoteDraft] Falha ao carregar rascunho remoto:", err);
  }
  return null;
}
