/**
 * Digitalização de Compressão Diametral — misturas asfálticas (DNER-ME
 * 138/94, sigla ASF.CD) e solo-cimento (DNIT 136/2010-ME, sigla
 * COMP.D[.dias]). Coleta de dados de campo na bancada, logo após a leitura
 * do QR. Mesmo padrão de `compressao-simples/ui.tsx`, sem o modo "completo"
 * (curva tensão×deformação) — as duas normas registram só a carga de
 * ruptura. Suporta mais de um corpo de prova (CP01, CP02...) — Gs fica em
 * nível de amostra, o resto é por CP.
 */
...
import { capsulaUmidadePct, parseCompressaoDiametralTag } from "./calc";
import type { CdAmostraTipo, CdCapsula, CdCargaUnidade, CdCorpoDeProva } from "./types";
import { newCdCapsula, newCdCorpoDeProva } from "./types";

// -------- Tipos do payload de campo (Compressão Diametral) --------
...
export interface CdFieldPayload {
  ident: {
    os: string;
    amostraCodigo: string;
    servicoNome?: string;
    tipoEnsaioNome: string;
    tipoEnsaioCodigo: string; // "ASF.CD" | "COMP.D"
...
export function emptyCdPayload(
  ident: CdFieldPayload["ident"],
  amostraTipo: CdAmostraTipo = "asfalto",
...
function draftKey(ident: CdFieldPayload["ident"]) {
  return `compressao-diametral-scan://${ident.os}/${ident.amostraCodigo}`;
}
function persistLocal(data: CdFieldPayload) {
  try {
    window.localStorage.setItem(draftKey(data.ident), JSON.stringify({ ...data, savedAt: new Date().toISOString() }));
  } catch { /* ignora quota */ }
}
function loadLocal(ident: CdFieldPayload["ident"]): CdFieldPayload | null {
  try {
    const raw = window.localStorage.getItem(draftKey(ident));
    if (!raw) return null;
    const p = JSON.parse(raw) as CdFieldPayload;
...
const AMOSTRA_TIPO_LABEL: Record<CdAmostraTipo, string> = {
  asfalto: "Mistura asfáltica",
  dosagem: "Dosagem (solo-cimento)",
};

export async function dispatchCompressaoDiametral(
  ...
  // A tag do QR já diz asfalto/dosagem (e a idade de cura, se dosagem) —
  // pré-preenche o formulário, mas o operador ainda confere/edita na tela.
  const parsedTag = parseCompressaoDiametralTag(servicoNome) ?? parseCompressaoDiametralTag(payload.ensaio_tag_codigo as string | undefined);
  const amostraTipo = parsedTag?.amostraTipo ?? "asfalto";
  const ident: CdFieldPayload["ident"] = {
    os,
    amostraCodigo,
    servicoNome,
    tipoEnsaioNome: `Compressão Diametral — ${AMOSTRA_TIPO_LABEL[amostraTipo]}`,
    tipoEnsaioCodigo: amostraTipo === "dosagem" ? "COMP.D" : "ASF.CD",
  ...
  const r = await criarPendenciaOuGuardar({
    data: {
      ...
      tipo_ensaio: "compressao-diametral",
      origem: "digitalizacao",
      payload: emptyCdPayload(ident, amostraTipo, parsedTag?.idadeCuraDias ?? null) as unknown as Record<string, unknown>,
    },
  });
  return { to: "/relatorio/digitalizacao/compressao-diametral", search: { pid: r.id } };
}

// -------- Editor mobile-first (usado após leitura do QR) --------
export function CompressaoDiametralWorkspace({
  initial,
  pendenciaId,
  onBack,
  officePhotos = [],
}: {
  initial: CdFieldPayload;
  pendenciaId: string | null;
  onBack: () => void;
  /** Fotos já adicionadas no relatório do escritório — só leitura aqui. */
  officePhotos?: Photo[];
}) {
  const [data, setData] = useState<CdFieldPayload>(initial);
  ...
  const comIndicesFisicos = isDosagem;
      ...
      if (!pidRef.current) {
        try {
          const r = await criarFn({
            data: { os: snap.ident.os, amostra: snap.ident.amostraCodigo || null, ensaio: snap.ident.tipoEnsaioNome, tipo_ensaio: "compressao-diametral", origem: "digitalizacao", payload },
          });
          pidRef.current = r.id;
          setPid(r.id);
  ...
  function updateCp(patch: Partial<CdCorpoDeProva>) {
    setData((d) => ({
      ...d,
      corposDeProva: d.corposDeProva.map((c, i) => (i === activeCp ? { ...c, ...patch } : c)),
    }));
  }
  function updateCapsula(i: number, p: Partial<CdCapsula>) {
    setData((d) => ({
      ...d,
  ...
  function addCp() {
    setData((d) => ({ ...d, corposDeProva: [...d.corposDeProva, newCdCorpoDeProva(`CP${String(d.corposDeProva.length + 1).padStart(2, "0")}`)] }));
  ...
  async function handlePhotos(fase: "antes" | "depois", files: FileList | null) {
    ...
    for (const f of Array.from(files)) {
      ...
      try {
        ...
        const photo: CdPhoto = { id: `p_${Math.random().toString(36).slice(2, 9)}`, dataUrl, bytes, fase, caption: "" };
  ...
  async function persistAndGetPid(): Promise<string | null> {
    ...
    try {
      const r = await criarFn({
        data: { os: data.ident.os, amostra: data.ident.amostraCodigo || null, ensaio: data.ident.tipoEnsaioNome, tipo_ensaio: "compressao-diametral", origem: "digitalizacao", payload },
      });
      setPid(r.id);
      return r.id;
    } catch (e: unknown) {
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto text-xs text-muted-foreground">
          {data.ident.tipoEnsaioCodigo} · Compressão Diametral
        </span>
      </div>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-2 gap-3">
          <div>
            <Label className="text-xs">Compressão diametral em</Label>
            <Select value={data.amostraTipo} onValueChange={(v) => { setData((d) => ({ ...d, amostraTipo: v as CdAmostraTipo })); queueMicrotask(saveToServer); }}>
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-xs">4 leituras de altura (espessura) e 4 de diâmetro (cm).</CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-xs">Pico de carga na ruptura por tração (compressão diametral).</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Select value={cp.picoCargaUnidade} onValueChange={(v) => { updateCp({ picoCargaUnidade: v as CdCargaUnidade }); queueMicrotask(saveToServer); }}>
                <SelectTrigger className="mt-1"><SelectValue /></SelectTrigger>
                <SelectContent>
              ...
              {officePhotos.map((p) => (
                <div key={p.id} className="relative rounded-md border overflow-hidden">
                  ...
                  <div className="p-1 text-[10px] text-center text-muted-foreground">
                    {p.kind === "moldagem" ? "Antes" : p.kind === "ruptura" ? "Após ruptura" : "Outro"}
                  </div>
                </div>
              ))}
            </div>
...
export function CompressaoDiametralPendenciaEditor({ pendenciaId, onBack }: { pendenciaId: string | null; onBack: () => void }) {
  const snapshotFn = useServerFn(getLabEnsaioSnapshot);
  // A do servidor ou, sem rede, a guardada no aparelho (lib/fila-offline.ts).
  const { pendencia, carregando } = usePendenciaDaBancada(pendenciaId, "compressao_diametral_scan_pendencias");
  const linked = (pendencia?.payload as unknown as CdFieldPayload | undefined)?._linkedEnsaio;
  const { data: officeSnapshot } = useQuery({
    queryKey: ["compressao_diametral_linked_ensaio_photos", linked?.osId, linked?.amostraId, linked?.ensaioId],
  ...
  const fallbackIdent: CdFieldPayload["ident"] = {
    os: pendencia?.os ?? "",
    amostraCodigo: pendencia?.amostra ?? "",
    tipoEnsaioNome: "Compressão Diametral",
    tipoEnsaioCodigo: "ASF.CD",
  };
  const initial: CdFieldPayload = pendencia?.payload
    ? (pendencia.payload as unknown as CdFieldPayload)
    : emptyCdPayload(fallbackIdent);
  const safe: CdFieldPayload = {
    ident: initial.ident ?? fallbackIdent,
    amostraTipo: initial.amostraTipo ?? "asfalto",
    ...
    corposDeProva: Array.isArray(initial.corposDeProva) && initial.corposDeProva.length
      ...
      : [newCdCorpoDeProva("CP01")],
    fotos: Array.isArray(initial.fotos) ? initial.fotos : [],
    obs: initial.obs ?? "",
  };
  return <CompressaoDiametralWorkspace initial={safe} pendenciaId={pendenciaId} onBack={onBack} officePhotos={officePhotos} />;
}

// -------- Bloco simples de fotos --------
function PhotoBlock({
  title, photos, onAdd, onRemove,
}: { title: string; photos: CdPhoto[]; onAdd: (files: FileList | null) => void; onRemove: (id: string) => void }) {
