...
import { UNPage } from "@/routes/_app.relatorio.umidade-natural";
import { LabEnsaioProvider, type LabEnsaioContextValue } from "@/features/lab/context";
import type { Photo } from "@/features/lab/types";

export const Route = createFileRoute("/_app/modelos-relatorios/umidade-natural")({
  ssr: false,
  component: ModeloUmidadeNatural,
  head: () => ({
    meta: [
      { title: "Modelo · Umidade Natural — Suporte INFRA" },
      {
        name: "description",
        content:
          "Modelo vivo do relatório de Umidade Natural (NBR 6457) com amostra fantasma para consulta administrativa.",
      },
    ],
  }),
});
...
function ModeloUmidadeNatural() {
  const [photos, setPhotos] = useState<Photo[]>(() => {
    try {
      const saved = localStorage.getItem("un_modelo_photos");
  ...
  const addPhoto = (photo: Omit<Photo, "id" | "createdAt">) => {
    ...
    setPhotos((prev) => {
      ...
      try {
        localStorage.setItem("un_modelo_photos", JSON.stringify(next));
      } catch {}
      return next;
    });
  };
