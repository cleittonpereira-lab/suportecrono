 ...
 * trabalho do ensaio de Compressão Diametral. Guarda o estado editável
 * (sample) por escopo (`ensaio.id` ou "local") sincronizando em tempo real
 * com controle de concorrência.
 */
import type { CompressaoDiametralSample } from "./types";
...
const KEY = (scopeId: string) => `compressao-diametral:draft:${scopeId}`;
...
export function saveDraft(
  ...
  const timer = setTimeout(() => {
    ...
    trackSave(() =>
      ...
      .catch((err) => {
        console.warn("[Compressão Diametral saveSharedDraft] Falha na sincronização:", err);
      });
  }, 400);
  ...
  try {
    ...
    const res = await loadSharedDraft({
      data: {
        ...
        osNum: opts?.osNum,
        amCode: opts?.amCode,
        ensaioTipo: opts?.ensaioTipo || "compressao-diametral",
      },
    });
    ...
    console.warn("[Compressão Diametral fetchRemoteDraft] Falha ao carregar rascunho remoto:", err);
  }
  return null;
}
