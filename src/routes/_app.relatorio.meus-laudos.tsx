...
export const Route = createFileRoute("/_app/relatorio/meus-laudos")({
  ssr: false,
  component: MeusLaudosPage,
  head: () => ({ meta: [{ title: "Meus laudos — Suporte INFRA" }] }),
});

const INFO: Record<
  Fila,
  { titulo: string; vazio: string; Icon: typeof Inbox; cor: string; barra: string; acao: string; sla: number | null }
> = {
  devolvido: {
  ...
  aprovado: {
    titulo: "Aprovados (30 dias)",
    vazio: "Nenhum laudo aprovado nos últimos 30 dias.",
    Icon: CheckCircle2,
...
function nomeDoEnsaio(r: EmissaoRow) {
  return (r.ensaio_tipo && ENSAIO_LABEL[r.ensaio_tipo as EnsaioTipo]) || r.ensaio_nome || r.ensaio_tipo || "Ensaio";
}

function MeusLaudosPage() {
  ...
  const [rows, setRows] = useState<EmissaoRow[] | null>(null);
  ...
  const recarregar = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Não foi possível carregar os laudos: " + (e instanceof Error ? e.message : String(e)));
    } finally {
      setCarregando(false);
    }
  }, [listFn]);
  ...
  // Tempo real: laudo enviado, verificado ou aprovado em outro computador.
  useEffect(() => {
    let timer: ReturnType<typeof setTimeout> | undefined;
    const parar = aoMudar((docs) => {
      if (!docs.some((d) => d.pasta === "lab-ensaios")) return;
      clearTimeout(timer);
      timer = setTimeout(() => void recarregar(), 500);
    });
    return () => {
      clearTimeout(timer);
      parar();
  ...
  const mesaCompleta = useMemo(() => montarMesa(rows ?? [], agora), [rows]); // eslint-disable-line react-hooks/exhaustive-deps

  const comigo = useMemo(() => {
    const out = {} as Record<Fila, number>;
    for (const f of FILAS) out[f] = mesaCompleta[f].filter((l) => pedeMinhaAcao(l, f, papel)).length;
    return out;
  }, [mesaCompleta, papel]);
  ...
  const papelTexto = papel.aprova ? "Você verifica e aprova" : papel.verifica ? "Você verifica" : "Você digita";

  return (
    <div className="flex flex-col gap-5 p-4 lg:p-6 pb-20">
      <PageHeader
        eyebrow="Relatório"
        icon={Inbox}
        title="Meus laudos"
        description="O fluxo de todos os laudos num lugar só: o que foi devolvido, o que falta verificar, o que falta aprovar e o que já saiu."
        actions={
          <Button variant="outline" size="sm" onClick={() => void recarregar()} disabled={carregando} className="gap-2">
          ...
          <div className="flex items-center gap-3 pr-2">
            <div
              className={cn(
                ...
                totalComigo > 0 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground",
              )}
            >
              {rows == null ? "…" : totalComigo}
            </div>
            <div>
              <div className="text-sm font-semibold">{totalComigo === 1 ? "laudo precisa de você" : "laudos precisam de você"}</div>
            ...
            {ORDEM_DO_QUADRO.filter((f) => f !== "aprovado").map((f) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={cn("ml-auto text-lg font-bold tabular-nums", comigo[f] > 0 ? I.cor : "text-muted-foreground")}>
                    {comigo[f]}
                  </span>
                </a>
              );
          ...
          <div className="mt-4">
            <div className="flex h-2 w-full overflow-hidden rounded-full bg-muted">
              {ORDEM_DO_QUADRO.map((f) => {
                ...
                return n > 0 ? (
                  <div key={f} className={INFO[f].barra} style={{ width: `${(n / total) * 100}%` }} title={`${INFO[f].titulo}: ${n}`} />
                ) : null;
              })}
            </div>
            <div className="mt-1.5 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-muted-foreground">
              {ORDEM_DO_QUADRO.map((f) => (
                <span key={f} className="inline-flex items-center gap-1">
                  ...
                  {INFO[f].titulo}: <b className="text-foreground tabular-nums">{mesaCompleta[f].length}</b>
                </span>
              ))}
            </div>
          </div>
      ...
      {/* Quadro */}
      {rows == null ? (
        <div className="rounded-lg border border-dashed p-10 text-center text-sm text-muted-foreground">Carregando laudos…</div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {ORDEM_DO_QUADRO.map((f) => {
            ...
            const lista = mesa[f];
            return (
              <section key={f} id={`fila-${f}`} className="flex min-w-0 flex-col rounded-xl border bg-muted/20">
                <header className="flex items-center gap-2 border-b px-3 py-2.5">
                  <span className={cn("h-2.5 w-2.5 rounded-full", I.barra)} />
                  <h2 className={cn("text-sm font-semibold", I.cor)}>{I.titulo}</h2>
                  <span className="ml-auto rounded-full bg-background px-2 py-0.5 text-xs font-bold tabular-nums">{lista.length}</span>
                </header>
      ...
      >
        <DialogContent className="flex h-[92vh] w-[95vw] max-w-[95vw] flex-col p-0">
          <DialogHeader className="border-b px-4 py-2">
            ...
            <DialogDescription className="text-xs">Cópia oficial do Drive, com as assinaturas já registradas.</DialogDescription>
          </DialogHeader>
    ...
    >
      ...
      {fila === "devolvido" && r.verification_comment && (
        <div className="mt-1.5 rounded bg-rose-500/10 px-2 py-1 text-[11px] text-rose-900 dark:text-rose-200">
          ...
          {r.verified_by_name && <span className="text-muted-foreground"> — {r.verified_by_name}</span>}
        </div>
      )}
      ...
      <div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[11px] text-muted-foreground">
        {fila === "aprovado" ? (
          ...
          <>
            <span className={cn(atrasado && "font-semibold text-destructive")} title={desde ? fmtData(desde) : ""}>
              há {fmtEspera(esperaMs)}
              {atrasado ? " · atrasado" : ""}
            </span>
            {r.requested_by_name && <span>· enviado por {r.requested_by_name}</span>}
            {fila === "aprovar" && r.verified_by_name && <span>· verificado por {r.verified_by_name}</span>}
          </>
        )}
      </div>
      ...
      <div className="mt-2 flex gap-1.5">
        {rota && (
          <Button asChild size="sm" variant={meu && fila !== "aprovado" ? "default" : "outline"} className="h-7 flex-1 text-xs">
        ...
        {temPdf && (
          <Button size="sm" variant="ghost" className="h-7 gap-1 text-xs" disabled={abrindo} onClick={onVerPdf}>
