...
import {
  ...
  Beaker, History, FileText, Plus, Trash2,
} from "lucide-react";
...
} from "@/features/compressao-diametral/report-versions";
import { sincronizarVersoesComDrive, useVersoesAoVivo } from "@/lib/revisoes-do-drive";
import { syncRevision, fetchDriveStatus } from "@/features/compressao-diametral/driveSync";
...
import type {
  CompressaoDiametralSample, CdAmostraTipo, CdCapsula, CdCorpoDeProva,
} from "@/features/compressao-diametral/types";
import { seedCompressaoDiametralSample, newCdCorpoDeProva } from "@/features/compressao-diametral/types";
import {
  capsulaUmidadePct, teorUmidadeMedio, calcCorpoDeProva, mediaCps,
} from "@/features/compressao-diametral/calc";
import { loadDraft, saveDraft, fetchRemoteDraft, flushDraft } from "@/features/compressao-diametral/draftStore";
import { listPendenciasDigitacao } from "@/lib/lab-pendencias.functions";
import { findMatchingPendencia } from "@/lib/pendencia-match";
import type { CdFieldPayload } from "@/features/compressao-diametral/ui";
...
const AMOSTRA_TIPO_LABEL: Record<CdAmostraTipo, string> = { asfalto: "Mistura Asfáltica", dosagem: "Dosagem (solo-cimento)" };

function normsFor(tipo: CdAmostraTipo): ReportNorm[] {
  if (tipo === "dosagem") {
    return [{ text: "DNIT 136/2010-ME - Pavimentação — Solo-cimento — Determinação da resistência à tração por compressão diametral" }];
  }
  return [{ text: "DNER-ME 138/94 - Misturas betuminosas — Determinação da resistência à tração por compressão diametral" }];
}

export const Route = createFileRoute("/_app/relatorio/compressao-diametral")({
  component: () => {
    const ctx = useOptionalLabEnsaio();
    return ctx?.ensaio ? <CompressaoDiametralPage /> : <EnsaioListByType tipo="compressao-diametral" />;
  },
  head: () => ({
    meta: [
      { title: "Compressão Diametral — Suporte INFRA" },
      {
        name: "description",
        content: "Resistência à tração por compressão diametral (ensaio brasileiro) em mistura asfáltica (DNER-ME 138/94) ou solo-cimento (DNIT 136/2010-ME).",
      },
    ],
  }),
});
...
function useCdResults(sample: CompressaoDiametralSample) {
  return useMemo(() => {
    const comIndices = sample.amostraTipo === "dosagem";
    const results = sample.corposDeProva.map((cp) =>
      calcCorpoDeProva(cp, { comIndices, gs: sample.massaEspecificaGraos }),
    );
    const media = results.length > 1 ? mediaCps(results) : null;
    return { comIndices, results, media };
  }, [sample]);
}

/** Página única do laudo: identificação + CP(s) + índices físicos (se dosagem) + resultado. */
function CompressaoDiametralReportPage({
  sample,
  photos: photosRecebidas = [],
}: {
  sample: CompressaoDiametralSample;
  photos?: import("@/features/lab/types").Photo[];
}) {
  const { comIndices, results, media } = useCdResults(sample);
  ...
  return (
    <ReportPage
      ...
      page={1}
      total={1}
      title={`COMPRESSÃO DIAMETRAL — ${AMOSTRA_TIPO_LABEL[sample.amostraTipo].toUpperCase()}`}
      ...
      <div className="space-y-2 text-[10px] text-[#141414]">
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#141414]/5 text-[8.5px] font-semibold">
                ...
                <td className="border border-[#141414] px-1 py-0.5 text-center">RT (kPa)</td>
                <td className="border border-[#141414] px-1 py-0.5 text-center">RT (MPa)</td>
              </tr>
            </thead>
            <tbody>
              {results.map((r, i) => (
                <tr key={r.id}>
                  ...
                  <td className="border border-[#141414] px-1 py-0.5 text-center font-semibold">{fmt(r.rtKPa, 0)}</td>
                  <td className="border border-[#141414] px-1 py-0.5 text-center font-semibold">{fmt(r.rtMPa, 3)}</td>
                </tr>
              ))}
              {media && (
                <tr className="bg-[#141414]/10">
                  ...
                  <td className="border border-[#141414] px-1 py-0.5 text-center font-bold">{fmt(media.rtKPa, 0)}</td>
                  <td className="border border-[#141414] px-1 py-0.5 text-center font-bold">{fmt(media.rtMPa, 3)}</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="border border-[#141414] px-2 py-1.5 flex items-center justify-between">
          <span className="text-[9.5px] font-semibold uppercase">
            Resistência à Tração por Compressão Diametral — RT {media ? "(média)" : ""}
          </span>
          <span className="text-[13px] font-bold">
            {fmt(media ? media.rtKPa : results[0]?.rtKPa, 0)} kPa &nbsp;·&nbsp; {fmt(media ? media.rtMPa : results[0]?.rtMPa, 3)} MPa
          </span>
        </div>
        ...
        <div className="border border-[#141414]">
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 p-2 text-[8.5px] leading-tight">
            <div><b>RT</b> — resistência à tração por compressão diametral (RT = 2P/(π·D·H))</div>
          ...
          <div className="border-t border-[#141414]/40 px-2 py-1 text-[8px] text-[#141414]/70 leading-tight">
            RT = 2 × Carga de ruptura / (π × Diâmetro × Altura do corpo de prova).
            {sample.amostraTipo === "asfalto" && " Conforme DNER-ME 138/94."}
            {sample.amostraTipo === "dosagem" && " Conforme DNIT 136/2010-ME."}
...
export function CompressaoDiametralPage() {
  const ctx = useOptionalLabEnsaio();
  ...
  const initialSample: CompressaoDiametralSample = useMemo(() => {
    const base = seedCompressaoDiametralSample();
  ...
  const [sample, setSample] = useState<CompressaoDiametralSample>(() =>
   ...
   * vazia mesmo com os PDFs já salvos no servidor.
   */
    ...
    try {
      ...
      const result = await withTimeout(
        syncRevision({
          ...
          ctxEnsaio: { tipo: "compressao-diametral", nome: sample.reportNumber }, fotos: fotosParaDrive(),
        }),
        25000,
  ...
  useEffect(() => {
    ...
    fetchRemoteDraft(scopeId, {
      ...
      ensaioTipo: "compressao-diametral",
    })
  ...
  // sobrescrever edição manual já feita no escritório).
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const fp = pend?.payload as unknown as CdFieldPayload | undefined;
        ...
        console.warn("[Compressão Diametral prefill] Falha:", err);
      }
    })();
  ...
  const updateSample = <K extends keyof CompressaoDiametralSample>(k: K, v: CompressaoDiametralSample[K]) =>
    setSample((s) => ({ ...s, [k]: v }));
  const cp = sample.corposDeProva[activeCp] ?? sample.corposDeProva[0];
  const updateCp = (patch: Partial<CdCorpoDeProva>) =>
    setSample((s) => ({ ...s, corposDeProva: s.corposDeProva.map((c, i) => (i === activeCp ? { ...c, ...patch } : c)) }));
  const updateCapsula = (idx: number, patch: Partial<CdCapsula>) =>
    setSample((s) => ({
      ...s,
  ...
  const addCp = () => {
    setSample((s) => ({ ...s, corposDeProva: [...s.corposDeProva, newCdCorpoDeProva(`CP${String(s.corposDeProva.length + 1).padStart(2, "0")}`)] }));
   ...
   * timeout). Foto que falhar interrompe a geração (é o laudo oficial) em
   * vez de sair silenciosamente sem a imagem.
   */
  ...
  const handleGeneratePdf = async () => {
    ...
    try {
      ...
      aEl.download = `COMP-DIAM_${base}.pdf`;
  ...
  const handleSaveVersion = async (opts?: { skipVerification?: boolean }) => {
    ...
    try {
      ...
      const base = (sample.workNumber || sample.os || "relatorio").toString().replace(/[^\w-]+/g, "_");
      const filename = `COMP-DIAM_${base}_Rev-${String(rev).padStart(2, "0")}.pdf`;
      ...
      await requestApproval({
        data: {
          ...
          index: {
            ...
            ensaio_tipo: "compressao-diametral", ensaio_nome: `Compressão Diametral — ${AMOSTRA_TIPO_LABEL[sample.amostraTipo]}`,
          },
        },
      });
   ...
   * abrir uma revisão nova nem mexer no status/fluxo.
   */
  const handleAtualizarPdfAtual = async () => {
    ...
    try {
      ...
      const blob = await buildReportPdfBlob();
      const base = (sample.workNumber || sample.os || "relatorio").toString().replace(/[^\w-]+/g, "_");
      const filename = `COMP-DIAM_${base}_Rev-${String(revAtual).padStart(2, "0")}.pdf`;
   ...
   * sempre calculados ao vivo a partir do histórico de aprovações.
   */
  ...
  const applyApprovalSideEffects = async (info: {
    ...
    try {
      ...
      console.warn("[Compressão Diametral] Falha ao regenerar PDF pós-aprovação:", err);
    }
  };
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between px-6 py-3 border-b bg-card">
            <div>
              <DialogTitle className="text-base font-bold text-foreground">
                Compressão Diametral — {AMOSTRA_TIPO_LABEL[sample.amostraTipo]} — Dados Atuais (não salvo)
              </DialogTitle>
          ...
          <div className="flex-1 min-h-0 overflow-auto bg-[#525659] p-8 flex justify-center">
            <div className="flex flex-col items-center gap-8 shrink-0 pb-12">
              <div className="w-[210mm] h-[297mm] shadow-2xl bg-white shrink-0 overflow-hidden">
                <CompressaoDiametralReportPage sample={{ ...sample, ...reportOverrides }} photos={ctx?.photos ?? []} />
              </div>
            </div>
          </div>
        </DialogContent>
      ...
      <div className="flex h-full flex-col bg-background p-4 lg:p-6 pb-20">
        <div className="mb-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              />
              <EnsaioTitleBlock
                title={`Compressão Diametral — ${AMOSTRA_TIPO_LABEL[sample.amostraTipo]}`}
                description="Resistência à tração por compressão diametral (ensaio brasileiro) — mistura asfáltica ou solo-cimento, conforme o tipo de amostra."
              />
            </div>
          </div>
        ...
        <Card className="mb-4 border-primary/30 bg-primary/5">
          ...
          <CardContent className="pt-0">
            <div className="grid gap-3 text-xs sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div>
                <div className="text-muted-foreground">RT {media ? "(média)" : ""}</div>
                <div className="font-medium">{fmt(media ? media.rtKPa : results[0]?.rtKPa, 0)} kPa · {fmt(media ? media.rtMPa : results[0]?.rtMPa, 3)} MPa</div>
              </div>
        ...
        <SampleEditDialog
          ...
          onSave={(updated) => {
            setSample((prev) => ({
              ...
              sampleState: (updated.sampleState as CompressaoDiametralSample["sampleState"]) || prev.sampleState,
            ...
            <TabsContent value="amostra" className="m-0 space-y-4">
              <Card className="mb-4">
                ...
                <CardContent className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                  <div>
                    <Label className="text-[10px] uppercase text-muted-foreground">Compressão diametral em</Label>
                    <Select value={sample.amostraTipo} onValueChange={(v) => updateSample("amostraTipo", v as CdAmostraTipo)}>
                      <SelectTrigger className="h-8 text-xs mt-1"><SelectValue /></SelectTrigger>
                      <SelectContent>
                        <SelectItem value="asfalto">Mistura Asfáltica</SelectItem>
                  ...
                  {sample.sampleState === "compactada" && (
                    <div className="col-span-full grid grid-cols-1 gap-3 sm:grid-cols-2">
                      <div>
                        ...
                        <Select value={sample.compactionEnergy ?? ""} onValueChange={(v) => updateSample("compactionEnergy", v as CompressaoDiametralSample["compactionEnergy"])}>
                ...
                <CardContent className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <Select value={cp.picoCargaUnidade} onValueChange={(v) => updateCp({ picoCargaUnidade: v as CompressaoDiametralSample["corposDeProva"][number]["picoCargaUnidade"] })}>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Resultado ({cp.label}): <strong className="text-foreground">
                      {fmt(results.find((r) => r.id === cp.id)?.rtKPa, 0)} kPa · {fmt(results.find((r) => r.id === cp.id)?.rtMPa, 3)} MPa
                    </strong>
                  </p>
                  {media && (
                    <p className="text-xs text-muted-foreground">
                      Média de {results.length} CPs: <strong className="text-foreground">{fmt(media.rtKPa, 0)} kPa · {fmt(media.rtMPa, 3)} MPa</strong>
                    </p>
                  )}
                </CardContent>
              </Card>
        ...
        >
          <CompressaoDiametralReportPage sample={sample} photos={ctx?.photos ?? []} />
        </div>
      </div>
    </>
  );
