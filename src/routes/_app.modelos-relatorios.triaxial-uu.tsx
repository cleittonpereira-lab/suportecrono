...
export const Route = createFileRoute("/_app/modelos-relatorios/triaxial-uu")({
  ssr: false,
  component: ModeloTriaxialUU,
  head: () => ({
    meta: [
      { title: "Modelo · Triaxial UU — Suporte INFRA" },
      {
        name: "description",
        content:
          "Modelo vivo do relatório de Triaxial UU com amostra fantasma para consulta administrativa.",
      },
    ],
  }),
});
...
function ModeloTriaxialUU() {
  const [photos, setPhotos] = useState<Photo[]>(() => {
    try {
      const saved = localStorage.getItem("triaxial_uu_modelo_photos");
  ...
  const addPhoto = (photo: Omit<Photo, "id" | "createdAt">) => {
    ...
    setPhotos((prev) => {
      ...
      try {
        localStorage.setItem("triaxial_uu_modelo_photos", JSON.stringify(next));
      } catch {}
      return next;
    });
  };
