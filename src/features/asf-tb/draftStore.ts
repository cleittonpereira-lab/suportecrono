/**
 * Rascunho de trabalho do ASF.TB: localStorage + rascunho compartilhado no
 * servidor, com controle de revisão entre computadores. Mesmo padrão do
 * ASF.DAP (`asf-dap/draftStore.ts`).
 */
import type { AsfTbSample } from "./types";
...
const KEY = (scopeId: string) => `asf-tb:draft:${scopeId}`;

export type AsfTbDraft = {
  sample: AsfTbSample;
  photos?: any[];
  savedAt: string;
...
export function loadDraft(scopeId: string): Partial<AsfTbDraft> | null {
  if (typeof window === "undefined") return null;
  try {
    const raw = window.localStorage.getItem(KEY(scopeId));
    if (!raw) return null;
    const parsed = JSON.parse(raw) as Partial<AsfTbDraft>;
...
export function saveDraft(
  ...
  const timer = setTimeout(() => {
    ...
    trackSave(() =>
      ...
      .catch((err) => {
        console.warn("[ASF.TB saveSharedDraft] Falha na sincronização:", err);
      });
  }, 400);
...
): Promise<Partial<AsfTbDraft> | null> {
  try {
    ...
    const res = await loadSharedDraft({
      data: {
        ...
        osNum: opts?.osNum,
        amCode: opts?.amCode,
        ensaioTipo: opts?.ensaioTipo || "asf-tb",
      },
    });
    if (res?.success && res.payload) {
      ...
      const payloadWithRev = { ...(res.payload as object), rev: res.rev } as Partial<AsfTbDraft>;
      try {
    ...
    console.warn("[ASF.TB fetchRemoteDraft] Falha ao carregar rascunho remoto:", err);
  }
  return null;
}
