...
import { PermVPage } from "@/routes/_app.relatorio.perm-v";
import { LabEnsaioProvider, type LabEnsaioContextValue } from "@/features/lab/context";
import type { Photo } from "@/features/lab/types";

export const Route = createFileRoute("/_app/modelos-relatorios/perm-v")({
  ssr: false,
  component: ModeloPermV,
  head: () => ({
    meta: [
      { title: "Modelo · Permeabilidade a Carga Variável (PERM.V) — Suporte INFRA" },
      {
        name: "description",
        content:
          "Modelo vivo do relatório de Permeabilidade a Carga Variável (ABNT NBR 14545:2021) com amostra fantasma para consulta administrativa.",
      },
    ],
  }),
});
...
function ModeloPermV() {
  const [photos, setPhotos] = useState<Photo[]>(() => {
    try {
      const saved = localStorage.getItem("permv_modelo_photos");
  ...
  const addPhoto = (photo: Omit<Photo, "id" | "createdAt">) => {
    ...
    setPhotos((prev) => {
      ...
      try {
        localStorage.setItem("permv_modelo_photos", JSON.stringify(next));
      } catch {}
      return next;
    });
  };
