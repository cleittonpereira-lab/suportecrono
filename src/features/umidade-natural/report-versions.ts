/**
 * Persistência simples de versões (revisões) do relatório de Umidade Natural em PDF usando IndexedDB.
 * Escopo por ensaio (`scopeId`), tipicamente o `ctx.ensaio.id`. Se não houver contexto,
 * usa a chave "local" — permanece útil, apenas não é agrupado por ensaio.
 */

const DB_NAME = "suporte-infra-report-versions-umidade";
...
export function viewVersion(v: ReportVersion) {
  ...
  // Se o pop-up foi bloqueado, faz fallback para download.
  if (!w) {
    downloadVersion(v);
    return;
  }
